...
class TimeSeriesViewer(QWidget):
    ...
    def __init__(self, max_points: int = 10000, parent: Optional[QWidget] = None) -> None:
        ...
        annotation_layout.addWidget(QLabel("Shortcuts: [ prev, ] next"))
    ...
    def _jump_to_annotation(self, direction: str) -> None:
        ...
        current_position = self._current_view_start()
        if direction == "next":
    ...
    def _current_view_start(self) -> float:
        view_box = self.plot_widget.getPlotItem().getViewBox()
        if view_box is None:
